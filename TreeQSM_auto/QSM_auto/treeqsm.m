...
function [QSM, out1, out2] = treeqsm(P,inputs)
  ...
  for i = 1:na
    ...
    for j = 1:ni
      ...
      if ~isempty(cylinder.radius)
        ...
        if inputs.plot >= 1
          if inputs.Tria
            [f1, f2] = plot_models_segmentations(P,cover2,segment2,cylinder,trunk,...
                triangulation);
            out1 = f1;
            out2 = f2;
          else
            [f1, f2] = plot_models_segmentations(P,cover2,segment2,cylinder);
            out1 = f1;
            out2 = f2;
          end
          if nd > 1 || na > 1 || ni > 1
